...
void rst::rasterizer::draw(pos_buf_id pos_buffer, ind_buf_id ind_buffer, col_buf_id col_buffer, Primitive type)
{
    ...
    for (int x = 0; x < width; x++) 
    {
        for (int y = 0; y < height; y++)
        {
            ...
            for (int i = 0; i < 4; i++)
                finalColor += frame_buf_SSAA[get_index(x, y)][i];
            finalColor /= 4;
            set_pixel(Eigen::Vector3f(x,y,1.0f), finalColor);
        }           
    }
                ...
                for (float i = 0.25; i < 1.0; i += 0.5) 
                {
                    for (float j = 0.25; j < 1.0; j += 0.5)
                    {
                        if (insideTriangle(x + i + 0.5, y + j + 0.5, t.v)) 
                        {
                            ...
                            if (min_depth < depth_buf_SSAA[get_index(x, y)][index]) 
                            {                 
                                frame_buf_SSAA[get_index(x, y)][index] = t.getColor();
                                depth_buf_SSAA[get_index(x, y)][index] = min_depth;
                            }
                        }
                        index++;
                    }
